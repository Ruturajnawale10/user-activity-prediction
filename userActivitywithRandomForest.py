...
def fix_data(training_data, training_classes):
    nm = SMOTE(random_state=25,k_neighbors=2)
    training_data = np.array(training_data)
    training_classes = np.array(training_classes)
    X_res, y_res = nm.fit_resample(training_data, training_classes)
    return X_res, y_res

x_train, x_test = fix_data(x_train, x_test)
...
#using RandomForestClassifier as the classification model
from sklearn.neighbors import RandomForestClassifier
rfc = RandomForestClassifier(n_estimators=7, class_weight="balanced")
